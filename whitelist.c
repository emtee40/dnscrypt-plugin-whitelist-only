                  ...
                  const char * const file)
{
    ...
    size_t     line_len;
    PassType   pass_type = PASSTYPE_UNDEFINED;
    int        ret = -1;
    ...
    *domain_substr_list_p = NULL;
    ...
    while (fgets(buf, (int) sizeof buf, fp) != NULL) {
        ...
        if (line[0] == '*' && line[line_len - 1] == '*') {
            line[line_len - 1] = 0;
            line++;
            pass_type = PASSTYPE_SUBSTRING;
        } else if (line[line_len - 1] == '*') {
            line[line_len - 1] = 0;
            pass_type = PASSTYPE_PREFIX;
        } else {
            if (line[0] == '*') {
                line++;
            }
        ...
        if (pass_type == PASSTYPE_SUFFIX) {
            if ((domain_rev_list = fpst_insert_str(domain_rev_list, line,
                                                   (uint32_t) pass_type)) == NULL) {
                free(line);
                break;
            }
        } else if (pass_type == PASSTYPE_PREFIX) {
            if ((domain_list = fpst_insert_str(domain_list, line,
                                               (uint32_t) pass_type)) == NULL) {
                free(line);
                break;
            }
        } else if (pass_type == PASSTYPE_SUBSTRING) {
...
substr_match(FPST *list, const char *str,
             const char **found_key_p, uint32_t *found_pass_type_p)
{
    size_t i;
...
{
    return "Only respond with whitelisted domains and IP addresses";
}
...
{
    return
        "This plugin returns a REFUSED response if the query name is not in\n"
        "a list of whitelisted names, or if none of the returned IP\n"
        "addresses happen to be in a list of whitelisted IPs.\n"
        "\n"
        "Plugin parameters:\n"
        "--domains=<file>\n"
        "--ips=<file>\n"
        ...
        "  libdcplugin_whitelist.la,--ips=/etc/wl-ips,--domains=/etc/wl-names"
        "\n"
...
dcplugin_init(DCPlugin * const dcplugin, int argc, char *argv[])
{
    ...
    if ((passing = calloc((size_t) 1U, sizeof *passing)) == NULL) {
        return -1;
    }
    dcplugin_set_user_data(dcplugin, passing);
    passing->fp = NULL;
    passing->domains = NULL;
    passing->domains_rev = NULL;
    passing->domains_substr = NULL;
    passing->ips = NULL;
    passing->ltsv = 0;
    optind = 0;
#ifdef _OPTRESET
    ...
    while ((opt_flag = getopt_long(argc, argv,
        ...
        switch (opt_flag) {
        case 'd':
            if (parse_domain_list(&passing->domains, &passing->domains_rev,
                                  &passing->domains_substr, optarg) != 0) {
                return -1;
            }
            break;
        case 'i':
            if (parse_ip_list(&passing->ips, optarg) != 0) {
                return -1;
            }
            break;
        case 'l':
            ...
            if ((passing->fp = fopen(optarg, "a")) == NULL) {
                return -1;
            }
            break;
        default:
...
{
    Passing *passing = dcplugin_get_user_data(dcplugin);

    if (passing == NULL) {
        return 0;
    }
    fpst_free(passing->domains, free_list);
    passing->domains = NULL;
    fpst_free(passing->domains_rev, free_list);
    passing->domains_rev = NULL;
    fpst_free(passing->domains_substr, free_list);
    passing->domains_substr = NULL;
    fpst_free(passing->ips, free_list);
    passing->ips = NULL;
    if (passing->fp != NULL) {
        fclose(passing->fp);
        passing->fp = NULL;
...
static int
log_blocked_rr(const Passing * const passing,
               ...
               const size_t client_addr_len)
{
    ...
    ltsv_prop(passing->fp, "time", passing);
    timestamp_fprint(passing->fp, passing->ltsv);
    putc_unlocked('\t', passing->fp);
    ltsv_prop(passing->fp, "host", passing);
    ip_fprint(passing->fp, client_addr, client_addr_len);
    putc_unlocked('\t', passing->fp);
...
apply_block_domains(DCPluginDNSPacket *dcp_packet, Passing * const passing,
                    ldns_pkt * const packet)
{
    ...
    do {
        ...
        str_reverse(rev);
        if (fpst_starts_with_existing_key(passing->domains_rev,
                                          rev, owner_str_len,
                                          &found_key, &found_pass_type)) {
            const size_t found_key_len = strlen(found_key);

            assert(found_pass_type == PASSTYPE_SUFFIX);
            ...
            if (found_key_len < owner_str_len) {
                ...
                if (owner_part_len > 0U && fpst_starts_with_existing_key
                    (passing->domains_rev, rev, owner_part_len,
                     &found_key, &found_pass_type)) {
                    const size_t found_key_len = strlen(found_key);
        ...
        if (fpst_starts_with_existing_key(passing->domains,
                                          owner_str, owner_str_len,
                                          &found_key, &found_pass_type)) {
            assert(found_pass_type == PASSTYPE_PREFIX);
            pass = 1;
            break;
        }
        if (passing->domains_substr != NULL &&
            substr_match(passing->domains_substr, owner_str,
                         &found_key, &found_pass_type)) {
            assert(found_pass_type == PASSTYPE_SUBSTRING);
            pass = 1;
            break;
        }
    } while (0);
    if (pass == 0) {
        ...
        if (found_pass_type == PASSTYPE_SUFFIX) {
            str_lcpy(rev, found_key, sizeof rev);
            str_reverse(rev);
            log_blocked_rr(passing, owner_str, found_pass_type,
...
static DCPluginSyncFilterResult
apply_block_ips(DCPluginDNSPacket *dcp_packet, Passing * const passing,
                ldns_pkt * const packet)
{
    ...
    for (i = (size_t) 0U; i < answers_count; i++) {
        ...
        if (fpst_str_starts_with_existing_key(passing->ips, answer_str,
                                              &found_key, &found_pass_type)) {
            if (!(found_pass_type == PASSTYPE_PREFIX ||
...
dcplugin_sync_pre_filter(DCPlugin *dcplugin, DCPluginDNSPacket *dcp_packet)
{
    ...
    if (passing->domains == NULL && passing->domains_rev == NULL &&
        passing->domains_substr == NULL) {
        return DCP_SYNC_FILTER_RESULT_OK;
    }
    ...
    if ((result = apply_block_domains(dcp_packet, passing, packet))
...
dcplugin_sync_post_filter(DCPlugin *dcplugin, DCPluginDNSPacket *dcp_packet)
{
    ...
    if (passing->ips != NULL &&
        (result = apply_block_ips(dcp_packet, passing, packet))
         != DCP_SYNC_FILTER_RESULT_OK) {
        ldns_pkt_free(packet);
        return result;
    }
